...
namespace Telemetry.Infrastructure.IntegrationTests.BackgroundJobs;
    ...
    : BaseIntegrationTest(factory)
{
    ...
    public async Task ProcessAsync_ShouldProcessValidMessageSuccessfully()
    {
        ...
        OutboxMessageProcessorService<TelemetryDbContext> service = GetRequiredService<OutboxMessageProcessorService<TelemetryDbContext>>();
    ...
    public async Task ProcessAsync_ShouldMarkAsPoisonMessage_WhenContentIsInvalidJson()
    {
        ...
        var outboxMessage = new OutboxMessage
        {
            Id = Guid.NewGuid(),
            OccurredOnUtc = DateTime.UtcNow,
            Type = typeof(RawTelemetryAddedDomainEvent).AssemblyQualifiedName!,
            Content = "[]"
        };
        ...
        processedMessage!.ProcessedOnUtc.Should().NotBeNull();
        processedMessage.Error.Should().NotBeNull();
        processedMessage.Error.Should().Contain("could not be converted");
    }
